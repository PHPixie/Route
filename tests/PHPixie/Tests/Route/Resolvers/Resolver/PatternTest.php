...
/**
 * @coversDefaultClass \PHPixie\Route\Resolvers\Resolver\Pattern
 */
abstract class PatternTest extends \PHPixie\Test\Testcase
{
    ...
    public function setUp()
    {
        ...
        $this->matcher = $this->quickMock('\PHPixie\Route\Matcher');
    }
    ...
    public function testConfig()
    {
        ...
        foreach($sets as $set) {
            ...
            for($i=0; $i<2; $i++) {
                $method = $set[0];
                $this->assertSame($set[2], $this->resolver->$method());
            }
        }
    }
    ...
    public function testPattern()
    {
        ...
        foreach(array('host', 'path') as $key => $name) {
            ...
            for($i=0; $i<2; $i++) {
                $method = $name.'Pattern';
                $this->assertSame($pattern, $this->resolver->$method());
            }
        }
    }
    ...
    public function testMissingPattern()
    {
        foreach(array('host', 'path') as $name) {
            ...
            $method = $name.'Pattern';
            for($i=0; $i<2; $i++) {
                $this->assertSame(null, $this->resolver->$method());
            }
        }
    }
    ...
    {
        return $this->quickMock('\PHPixie\Route\Matcher\Pattern');
    }
